////////////////////////////////////
export interface TeamMember {
  name: string;
  role: string;
  description: string;
  image: string;
  social: {
    instagram?: string;
    twitter?: string;
    youtube?: string;
  };
}

export const team: TeamMember[] = [
  {
    name: "Lisa Mango",
    role: "Nutrition Expert",
    description:
      "Proin pretium, leo ac pellentesque mollis, felis nunc ultrices eros, sed gravida augue augue mollis justo. Donec elit libero, sodales nec, volutpat a, suscipit non.",
    image: "/images/teamintro1.jpg",
    social: {
      instagram: "https://instagram.com/angela",
      twitter: "https://twitter.com/angela",
      youtube: "https://youtube.com/@angela"
    }
  },
  {
    name: "Angela Smith",
    role: "Yoga Teacher",
    description:
      "Proin pretium, leo ac pellentesque mollis, felis nunc ultrices eros, sed gravida augue augue mollis justo. Donec elit libero, sodales nec, volutpat a, suscipit non.",
    image: "/images/teamintro2.jpg",
    social: {
      instagram: "https://instagram.com/david",
      twitter: "https://twitter.com/david",
      youtube: "https://youtube.com/@david"
    }
  },
  {
    name: "Jess Spark",
    role: "Yoga Teacher",
    description:
      "Proin pretium, leo ac pellentesque mollis, felis nunc ultrices eros, sed gravida augue augue mollis justo. Donec elit libero, sodales nec, volutpat a, suscipit non.",
    image: "/images/teamintro3.jpg",
    social: {
      instagram: "https://instagram.com/maria",
      twitter: "https://twitter.com/maria",
      youtube: "https://youtube.com/@maria"
    }
  }
];
/////////////////////////////////////////////////////////////////

